...
INT8U OSMutexCreate (OS_EVENT **event, INT8U HigherPriority)
{
  ...
  pont_event->OSEventCnt     = 1;                        // Set mutex counter value
...
INT8U OSMutexPost(OS_EVENT *pont_event)
{
  ...
  if (pont_event->OSEventGrp != 0)                    // See if any task is waiting for mutex
  {
    for(i=0;i<OS_EVENT_TABLE_SIZE;i++)
    {
	    if (pont_event->OSEventTbl[i] != 0)
  ...
  if (pont_event->OSEventCnt < 65535u)            // Make sure mutex will not overflow
  {            
    pont_event->OSEventCnt++;                     // Increment mutex count
